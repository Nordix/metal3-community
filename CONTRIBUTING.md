...
Commit message should contain signed off section with full name and email.
The presence of this sign-off indicates your acceptance of the DCO.
For example:
...
1. Ensure your commit history is clean and meaningful. Avoid commits with messages
   like "fix tests" or "incorporate review feedback". Consider squashing such
   commits, but note that splitting logically distinct changes into separate
   commits can make reviews easier and history more useful. When in doubt, ask
   the reviewers to help you determine the best commit structure.
...
AI agents contributing to Metal3 projects will automatically reference the
`AGENTS.md` file in each repository (if available) for detailed guidance on
that project's structure and conventions.
