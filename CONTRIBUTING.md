...
Here's a consolidated link of good first issues across the Metal3.io org.

- [Good first issues](https://github.com/search?q=org%3Ametal3-io%20label%3A%22help%20wanted%22%20is%3Aissue%20is%3Aopen&type=issues)
...
1. If your PR has multiple commits, you must
...
In Metal3.io, we generally run Kubernetes Prow tests and Jenkins tests in CI.
Prow tests are usually run automatically and jenkins tests are triggered
manually. You can read more on the [Metal3.io prow configuration](https://github.com/metal3-io/project-infra/blob/main/prow/README.md)
...
repo. Please make use of those to run the tests locally before push.
...
You will find more information on the review process in #merge-approval, #issue-and-pull-request-management,
and #commands-and-workflow section.
...
See individual repositories README.md and CONTRIBUTING.md files for:
...
Vulnerability fixes may also be done exceptionally.
...
After making changes locally, you can run `make serve` in the metal3-docs
repository which will build the HTML version and start a web server,
so you can preview if the changes render correctly at <http://localhost:3000>;
the preview auto-updates when changes are detected.

Note: you don't need to have mdBook installed, `make serve` will ensure
appropriate binaries for mdBook and any used plugins are downloaded into
the `hack/tools/bin/` directory.
...
Some Metal3 repositories include an `AGENTS.md` file that provides specific
